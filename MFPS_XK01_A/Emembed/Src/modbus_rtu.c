...
void Modbus_Fun3()
{
   ...
   for( uint16_t i = 0; i < 6; i++)
   {
       switch (i)
       {
        ...
        case 4:
            gui_info.alarm_temp2_value = rs485.rcvbuf[start_addr_03];
            gui_info.alarm_temp1_value = rs485.rcvbuf[start_addr_03 + 1];

            break;   

        case 5:
            gui_info.alarm_temp3_value = rs485.rcvbuf[start_addr_03 + 1];
...
void Modbus_Fun4()
{
    ...
    float current1,current2,current3 = 0;
    ...
    for( uint16_t i = 0; i < 6; i++)
    {
        switch (i)
        {
            ...
            case 3:
                current1 = (rs485.rcvbuf[start_addr_04]<<8) | rs485.rcvbuf[start_addr_04 + 1];  
                if(current1 <= 1360)
                {
                    current1 = 1360;
                }
                gui_info.current_24V = ((current1 - 1360) / 36.0);

                break;

            case 4:
                current2 = (rs485.rcvbuf[start_addr_04]<<8) | rs485.rcvbuf[start_addr_04 + 1];      
                if(current2 <= 1360)
                {
                    current2 = 1360;
                }
                gui_info.current_36V = ((current2 - 1360) / 36.0);

                break;

            case 5:
                current3 = (rs485.rcvbuf[start_addr_04]<<8) | rs485.rcvbuf[start_addr_04 + 1];  
                if(current3 <= 1360)
                {
                    current3 = 1360;
                }    
                gui_info.current_42V = ((current3 - 1360) / 36.0);
...
{
    uint8_t send_buf[8] = {0x35,0x03,0x00,0x00,0x00,0x06,0xBC,0xC1};
...
{
    uint8_t send_buf[8] = {0x35,0x04,0x00,0x00,0x00,0x06,0x7C,0x74};
...
void send_to_slave( void )
{
   ...
   send_buf[15] = gui_info.alarm_temp2_value;
   send_buf[16] = gui_info.alarm_temp1_value;

   send_buf[17] = 0x00;
   send_buf[18] = gui_info.alarm_temp3_value;
